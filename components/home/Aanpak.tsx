        ...
        <div className="max-w-[720px]">
          ...
          <Verschijn inView vertraging={0.2}>
            <p className="mt-5 text-[15.5px] leading-[1.65] text-[var(--fg-dim)] sm:text-[17px]">
              Het eerste gesprek is altijd vrijblijvend. Zo ja, dan komt er een voorstel: één agent,
              vaste prijs per fase, en elke fase eindigt in iets dat werkt. Geen pilot van zes maanden
              zonder resultaat.
            </p>
          </Verschijn>
        </div>
        <ol ref={ref} className="relative lg:pt-2">
          <span className="absolute left-[25px] top-2 bottom-2 w-px bg-[var(--border)] sm:left-[35px]" aria-hidden />
          <motion.span
          ...
          {STAPPEN.map(([titel, body], i) => (
            <Stap key={titel} index={i} titel={titel} body={body} p={p} reduced={reduced} />
          ))}
        </ol>
      </div>
    </section>
...
function Stap({
  ...
  const kleur = useTransform(aan, [0, 1], ['rgba(244,241,236,0.45)', 'rgba(233,180,106,1)']);
  const rand = useTransform(aan, [0, 1], ['rgba(255,255,255,0.14)', 'rgba(233,180,106,0.9)']);
  return (
    <Verschijn inView vertraging={0.06 * index}>
      <li className="relative grid grid-cols-[52px_minmax(0,1fr)] gap-x-4 py-5 sm:grid-cols-[72px_minmax(0,1fr)] sm:py-6">
        <motion.span
          ...
          style={reduced ? { color: 'rgba(233,180,106,1)', borderColor: 'rgba(233,180,106,0.9)' } : { color: kleur, borderColor: rand }}
        >
