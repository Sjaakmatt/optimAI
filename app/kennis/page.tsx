...
export default function KennisPage() {
  return (
    <SitePage>
      <section className="mx-auto max-w-[1080px] px-6 sm:px-10 pt-16 pb-12">
        <div className="max-w-[720px]">
          ...
          <h1 className="mt-4 font-display text-[40px] sm:text-[52px] leading-[1.05] tracking-tight text-[var(--ink)]">
            Gedachten,<br />
            <span className="italic text-[var(--ink-dim)]">zonder jargon.</span>
          </h1>
      ...
      <section className="mx-auto max-w-[1080px] px-6 sm:px-10 pb-20 divide-y divide-[var(--paper-edge)] border-y border-[var(--paper-edge)]">
        {POSTS.map((p) => (
          <Link
            key={p.slug}
            href={`/kennis/${p.slug}`}
          ...
          >
            <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-6">
              <div className="max-w-[660px]">
                ...
                <h2 className="mt-2 font-display text-[26px] leading-[1.15] text-[var(--ink)] group-hover:text-[var(--oker-deep)] transition-colors">
                  {p.title}
                </h2>
                <p className="mt-3 text-[15px] leading-[1.6] text-[var(--ink-dim)]">{p.lede}</p>
